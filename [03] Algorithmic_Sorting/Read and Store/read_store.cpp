...
int main()
{
    ...
    vector<double> numbers; // Create vector
    ...
    ofstream binaryFile("num_file.bin",ios::binary); // Create binary file
    if(!binaryFile){
        cerr << "Error creating the binary file!" << endl;
        return 1;
    }

    binaryFile.write(reinterpret_cast<char*>(numbers.data()), numbers.size() * sizeof(double));
